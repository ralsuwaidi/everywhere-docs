...
Note: Even though the system uses email for authentication, the payload key is "username" due to DRF's default `obtain_auth_token` view.
...
1. The token endpoint is provided by DRF's `obtain_auth_token` view:  
    ```
...
## Configuration
Ensure that `rest_framework.authtoken` is included in your `INSTALLED_APPS` in `settings.py`:
```
